...
export default function requestStudy({ navigation }) {
    ...
    function Request(studyName, intro, subject, people, day, locate, term, explain, wish, user) {
        const inputlist = [studyName, intro, subject, people, day, locate, term, explain, wish, user]
        const ref = firebase.database().ref().child('requestStudy');
        const chat_ref = firebase.database().ref().child("chat");
        ...
        if (inputlist.includes('')) {
            ...
            chat_ref.child(newchat.key).set({
                auth: {
                    host: {
                        id: user
                    }
            ...
            alert("스터디가 생성 되었습니다.")
            navigation.navigate("ChatPage",{key : newchat.key})
        }
    ...
    return (
        <View style={styles.container}>
            <ScrollView showsVerticalScrollIndicator={false}>
                ...
                <TouchableOpacity
                    style={styles.button}
                    onPress={() => Request(studyName, studyIntro, studySubject, studyPeople, studyDay, studyLocate, studyTerm, studyExplain, studyWish, getName)}>
